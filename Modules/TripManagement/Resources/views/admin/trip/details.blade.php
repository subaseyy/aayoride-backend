                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="px-xl-3">
                                            <dl class="data-list gy-2 text-dark">
                                                ...
                                                <dd class="m-0">
                                                    +{{ set_currency_symbol($trip->pickup_bonus + 0) }}</dd>

                                                    <?php
                                                    $totalAmount = $trip->actual_fare + ($trip?->fee?->delay_fee ?? 0) + ($trip?->fee?->idle_fee ?? 0) + ($trip?->fee?->cancellation_fee ?? 0) - ($trip->coupon_amount ?? 0) - ($trip->discount_amount ?? 0) + ($trip->pickup_bonus ?? 0);
                                                    ?>

                                                <dt>
{{ translate('VAT/Tax') }}
    <small class="font-semi-bold">
        <strong>
            @if($totalAmount != 0)
                ({{ round((($trip?->fee?->vat_tax ?? 0) * 100) / $totalAmount) }}%)
            @else
                0%
            @endif
        </strong>
                                                ...
                                                <dd class="m-0">
                                                    <strong>{{ set_currency_symbol($totalAmount) }}</strong>
                                                </dd>
                                            </dl>
                                        </div>
